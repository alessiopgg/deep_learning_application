...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train REINFORCE on CartPole-v1"
    )
...
def save_results(
    ...
    config = {
        ...
        "activation": "ReLU",
        "best_evaluation_episode": best_checkpoint["episode"],
        "best_evaluation_reward": best_checkpoint["average_reward"],
    }
    ...
    # Final policy after the last training episode
    torch.save(
        policy.state_dict(),
        output_dir / "policy.pt",
    )

    # Best policy according to periodic evaluation reward
    torch.save(
...
def main():
    ...
    if args.eval_every > args.episodes:
        raise ValueError(
            "--eval-every must be less than or equal to --episodes."
        )
